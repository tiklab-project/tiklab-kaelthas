package io.thoughtware.kaelthas.collection.common;
...
import io.thoughtware.kaelthas.collection.node.NodeItem;
import io.thoughtware.kaelthas.collection.serviceItem.ServiceItem;
import io.thoughtware.kaelthas.collection.utils.ConversionAllTypeUtil;
...
public class GetConfigAndReportData {
    ...
    public void getKubernetesInfo() throws ApiException {
        ...
        for (KubernetesMonitor kubernetesMonitor : monitorList) {
            try {
                CompletableFuture<History> future = CompletableFuture.supplyAsync(() -> getHistory(kubernetesMonitor), executor);
                futures.add(future);
            } catch (Exception e) {
        ...
        if (historyList.size() > 300) {
            ...
            executor.submit(() -> historyService.insertForList(list));
            historyList.clear();
        }
    }
    ...
    private static History getHistory(KubernetesMonitor kubernetesMonitor) {
        ...
        history.setMonitorId(kubernetesMonitor.getId());
        history.setGatherTime(ConversionAllTypeUtil.getDataTimeNow());
        history.setExpression(kubernetesMonitor.getExpression());

        try {

            getClusterData(kubernetesMonitor, history, api, appsV1Api);

            NodeItem.getNodeReportData(kubernetesMonitor, history, api, appsV1Api);

            ServiceItem.getNServiceReportData(kubernetesMonitor, history, api, appsV1Api);
